...
- Gamma GSAP coverage (tick 113): kinetic cells raise ScrollTrigger craft from ≥5 to ≥7 — nameplate and aurora ledger join the GSAP set
...
- Mobile-safe floor (tick 123): every experiment `<main>` clips horizontal overflow; site viewport is device-width; wide min-width cards must sit on an overflow-x rail — award craft that only works on desktop fails the lab
