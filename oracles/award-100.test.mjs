...
test("Fail-then-pass: Gamma kinetic ships GSAP in ≥9 cells and stays ≥110 lines", () => {
  ...
  assert.ok(
    withGsap.length >= 9,
    `RED: need ≥9 Gamma cells using GSAP, got ${withGsap.length}: ${withGsap.join(", ") || "none"}`,
  );
});
...
test("Fail-then-pass: Beta editorial ships Lenis on ≥12 cells and stays ≥110 lines", () => {
  ...
  assert.ok(
    withLenis >= 12,
    `RED: need ≥12 Beta editorial cells with Lenis SmoothScroll, got ${withLenis}`,
  );
});
