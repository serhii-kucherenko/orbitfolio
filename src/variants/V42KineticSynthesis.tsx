...
/** Kinetic Synthesis — deepened award cell */
export function Variant() {
  const _reduce = useReducedMotion() ?? false;
  return (
    <main className="min-h-screen p-3 md:p-8" style={{ background: "#f0fdf4", color: "#14532d" }}>
      <div className="mx-auto max-w-5xl rounded-[2rem] border p-6 md:p-12" style={{ borderColor: "#15803d66", background: "#ffffffcc" }}>
        <p className="text-[10px] uppercase tracking-[0.4em] opacity-55">Kinetic Synthesis</p>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <a href={`mailto:${cv.email}`} className="rounded-full border px-5 py-2.5 text-sm font-semibold" style={{ borderColor: "#15803d", color: "#15803d" }}>Book a chat</a>
          <ContactRow />
        </div>
        <div className="mt-10 grid gap-3 sm:grid-cols-4">
          {cv.highlights.map((h) => (
            <div key={h.label} className="rounded-2xl border p-4" style={{ borderColor: "#15803d40" }}>
