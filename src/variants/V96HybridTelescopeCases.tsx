...
/** Hybrid Telescope Cases — deepened award cell */
export function Variant() {
  const _reduce = useReducedMotion() ?? false;
  return (
    <main className="min-h-screen md:grid md:grid-cols-[280px_1fr]" style={{ background: "#fef3c7", color: "#78350f" }}>
      <aside className="border-r p-8 md:sticky md:top-0 md:h-screen" style={{ borderColor: "#b4530933", background: "#ffffff80" }}>
        <p className="text-[10px] uppercase tracking-[0.3em] opacity-60">Hybrid Telescope Cases</p>
        ...
        <a href={`mailto:${cv.email}`} className="mt-8 inline-block rounded-full px-4 py-2 text-xs font-bold" style={{ background: "#b45309", color: "#fff"}}>Email</a>
      ...
      <div className="px-6 py-16 md:px-12">
        ...
        <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {cv.highlights.map((h) => (
            <div key={h.label} className="rounded-xl border p-4" style={{ borderColor: "#b4530944" }}>
