...
/** Code Rain Archive — deepened award cell */
  ...
  return (
    <main className="min-h-screen" style={{ background: "#1c1917", color: "#fafaf9" }}>
      <section className="mx-auto grid max-w-6xl gap-10 px-6 pb-12 pt-28 md:grid-cols-[1.2fr_0.8fr]">
        <div>
          <motion.p initial={reduce ? false : { opacity: 0 }} animate={{ opacity: 1 }} className="text-[10px] uppercase tracking-[0.35em] opacity-60">Code Rain Archive</motion.p>
          ...
          <a href={`mailto:${cv.email}`} className="mt-8 inline-flex rounded-full px-5 py-2.5 text-sm font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2" style={{ background: "#fb923c", color: "#041016"}}>Hire conversation</a>
          <ContactRow className="mt-6 text-white/70" />
        </div>
        <div className="grid grid-cols-2 gap-3 content-start">
          {cv.highlights.map((h) => (
            <div key={h.label} className="rounded-2xl border p-4" style={{ borderColor: "#fb923c55" }}>
              <p className="text-3xl font-bold" style={{ color: "#fb923c" }}>{h.value}</p>
